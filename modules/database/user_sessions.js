...
function set_spotify_access(spotify_access, id) {
  const index = user_sessions.findIndex(
    (inner_obj) => inner_obj.spotify_id === id
  );
...
function user_github_exists(spotify_id) {
  if (
    get_user(spotify_id).github_auth === "" ||
    get_user(spotify_id).github_auth === undefined
  ) {
    return false;
  }
  return true;
...
function user_id_exists(spotify_id) {
  if (
    user_sessions.find((inner) => inner.spotify_id === spotify_id) === undefined
  ) {
    return false;
  }
  return true;
...
function remove_user(state) {
  ...
  const index_to_remove = user_sessions.findIndex(
    (inner_obj) => inner_obj.state === state
  );
  user_sessions.splice(index_to_remove, 1);
}

function remove_user_with_id(id) {
  const index_to_remove = user_sessions.findIndex(
    (inner_obj) => inner_obj.id === id
  );
  user_sessions.splice(index_to_remove, 1);
}
...
module.exports = {
  ...
  remove_user_with_id: remove_user_with_id,
};
