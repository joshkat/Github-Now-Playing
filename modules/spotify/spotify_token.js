  ...
  return (req, res, next) => {
    ...
    const post_body = querystring.stringify({
      ...
      redirect_uri: process.env.REDIRECT_URL
        ? `${process.env.REDIRECT_URL}/spotify_code`
        : "http://localhost:3000/spotify_code",
    });
    const options = {
      method: "POST",
      headers: {
    ...
    const token_request = https.request(
      token_endpoint,
      options,
      (token_response) => {
