...
function get_now_playing(id, counter = 0) {
  ...
  https
    .request(np_endpoint, options, (res) => {
      stream_to_message(res, (body) => {
        if (body.length === 0) {
          ...
          if (np_json.error) {
            ...
            const full_string = filter.clean(
              `${artist} - ${song}`.length > 80
                ? `${artist} - ${song}`.slice(0, 77) + "..."
                : `${artist} - ${song}`
            );
            if (!full_string.includes("*")) {
              update_github_status(
                full_string.length > 80
                  ? full_string.slice(0, 77) + `...`
                  : full_string,
                user_sessions.get_github_auth(id)
              );
            }
            const date = new Date();
            console.log(
              `\x1b[32m${id}\x1b[0m is currently streaming \x1b[34m${full_string}\x1b[0m @ \x1b[35m${date.getHours()}:${date.getMinutes()}:${date.getSeconds()} \x1b[0m`
            ); //<id> is currently streaming <song_name> @ <date>
          }
        }
        setTimeout(() => {
          get_now_playing(id);
